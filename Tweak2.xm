...
__attribute__((unused)) static int CellCount(UITableView *tableView) {
	int ret = 0;
...
- (void)_tl_tappedTableView:(UIGestureRecognizer *)recognizer {
	...
	if (indexPath != nil) {
		...
		if ([recognizer isKindOfClass:[UILongPressGestureRecognizer class]]) {
			...
			CGFloat height = [rowData heightForTable];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if ([indexPath section] == [tableView numberOfSections]-1 && [indexPath row] == (collapsed_sections[[indexPath section]] ? 0 : 1)) {
		if ([[tableView tableFooterView] frame].size.height == 0.f)
